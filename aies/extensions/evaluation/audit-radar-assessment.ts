...
function buildRules(): DimensionRule[] {
  return [
    ...
    },
    {
      ...
      protocolGapChecks: (evidence) => evidence.some((item) => item.evidenceId === "audit-evaluation-audit-radar-guidance-experiment-report")
        ? ["The audit can now turn posture-level review into a bounded next experiment, but it still needs repeated real execution of those planned non-baseline probes before the planner should justify stronger policy shifts."]
      ...
      highestLeverageNextStep: (evidence) => evidence.some((item) => item.evidenceId === "audit-evaluation-audit-radar-guidance-experiment-report")
        ? "Exercise the bounded guidance-experiment plan on repeated relevant guided cycles so evaluation can compare whether the planned non-baseline probes actually beat baseline instead of only producing a new plan artifact."
        : evidence.some((item) => item.evidenceId === "audit-evaluation-audit-radar-guidance-learning-review-report")
