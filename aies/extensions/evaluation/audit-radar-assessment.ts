...
function buildRules(): DimensionRule[] {
  return [
    ...
    },
    {
      ...
      protocolGapChecks: (evidence) => evidence.some((item) => item.evidenceId === "audit-evaluation-audit-radar-guidance-experiment-review-report")
        ? ["The audit can now review whether a bounded posture experiment is underway or complete, but it still needs repeated real execution of those planned non-baseline probes before the review should justify stronger policy shifts."]
      ...
      highestLeverageNextStep: (evidence) => evidence.some((item) => item.evidenceId === "audit-evaluation-audit-radar-guidance-experiment-review-report")
        ? "Exercise the bounded guidance-experiment plan on repeated relevant guided cycles until the experiment review reaches completion and can compare whether the planned non-baseline probes actually beat baseline instead of only producing an in-progress signal."
        : evidence.some((item) => item.evidenceId === "audit-evaluation-audit-radar-guidance-experiment-report")
