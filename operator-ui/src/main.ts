...
type ActionQueueView = "sync" | "followups" | "notes" | "all";
...
function classifyAction(item: OperatorActionItem): Exclude<ActionQueueView, "all"> {
  if (item.category === "manual-note") {
    return "notes";
  }
  if (["verification-followup", "recovery", "openspec-stale"].includes(item.category)) {
    return "followups";
  }
  return "sync";
}
...
class AiesOperatorApp extends LitElement {
  ...
  declare actionView: ActionQueueView;
  ...
  private expandedCards: Record<string, boolean> = {};
  ...
  private actionItems(items: OperatorActionItem[]): OperatorActionItem[] {
    if (this.actionView === "all") {
      return items;
    }
    return items.filter((item) => classifyAction(item) === this.actionView);
  }

  private actionCounts(items: OperatorActionItem[]): Record<ActionQueueView, number> {
    return items.reduce<Record<ActionQueueView, number>>((counts, item) => {
      counts.all += 1;
      counts[classifyAction(item)] += 1;
      return counts;
    }, {
      sync: 0,
      followups: 0,
  ...
  private actionViewLabel(view: ActionQueueView): string {
    switch (view) {
      case "sync":
        return "UI Sync Warnings / Logs";
      case "followups":
  ...
  private actionViewDescription(view: ActionQueueView): string {
    switch (view) {
      case "sync":
        return "Operator UI mutations, session sync attempts, and warning/failure logs generated by the console.";
      case "followups":
        return "System-generated debt that still needs attention, such as verification follow-up, recovery debt, or stale OpenSpec context.";
      case "notes":
        return "Manual operator notes and reminders created from this panel.";
      default:
        return "Combined view of sync logs, system follow-up debt, and manual operator notes.";
    }
  }

  private isCardExpanded(cardId: string, defaultOpen: boolean): boolean {
    return this.expandedCards[cardId] ?? defaultOpen;
  }

  private setCardExpanded(cardId: string, open: boolean): void {
    this.expandedCards = {
      ...this.expandedCards,
      [cardId]: open,
    };
  ...
  private renderCollapsibleCard(input: {
    id: string;
    title: string;
    meta: string;
    preview: string;
    ...
    return html`
      <section class="${input.cardClass ?? "card"} collapsible-card">
        <details
          class="collapsible-details"
          ?open=${this.isCardExpanded(input.id, input.defaultOpen ?? false)}
          @toggle=${(event: Event) => { this.setCardExpanded(input.id, (event.currentTarget as HTMLDetailsElement).open); }}
        >
          <summary class="collapsible-summary">
            <div class="row wrap">
              <div class="stack collapsible-heading-block">
                <div class="row wrap">
                  <div class="collapsible-heading">${input.title}</div>
                  <div class="timeline-meta">${input.meta}</div>
                </div>
                <div class="summary collapsible-preview">${input.preview}</div>
              </div>
          ...
          <div class="collapsible-body stack">${input.body}</div>
        </details>
      </section>
    `;
  }
  ...
  private renderPanel(panel: PanelState): TemplateResult {
    return this.renderCollapsibleCard({
      id: `panel:${panel.id}`,
      title: panel.title,
      meta: panel.provenance.sourceType,
  ...
  private renderThoughtStream(items: ThoughtStreamItem[]): TemplateResult {
    return this.renderCollapsibleCard({
      id: "main:thought-stream",
      title: "Cycle Thought Stream",
      meta: `${items.length} blocks`,
      preview: items[0]?.text ?? "No thought blocks captured for the selected session yet.",
      defaultOpen: true,
      body: html`
        <div class="callout small">
  ...
  private renderTimeline(events: OperatorTimelineEvent[]): TemplateResult {
    ...
    return this.renderCollapsibleCard({
      ...
      meta: `${items.length} visible / ${events.length} total`,
      preview: items[0]?.summary ?? "No timeline events match the current filter.",
      defaultOpen: true,
      body: html`
  ...
  private renderLive(state: OperatorStateResponse): TemplateResult {
    ...
    const actionCounts = this.actionCounts(state.actionQueue);
    const visibleActionItems = this.actionItems(state.actionQueue);
    const openRequestCount = state.requests.filter((item) => item.status === "open" || item.status === "approved").length;
    const latestTranscriptPreview = transcriptItems[transcriptItems.length - 1]?.text ?? "No user/assistant messages in the selected session yet.";
    return html`
      <div class="content">
        <div class="live-layout">
          <div class="center-column">
            ...
            ${this.renderCollapsibleCard({
              id: "main:messages",
              title: "Messages",
              meta: `${transcriptItems.length} messages`,
              preview: latestTranscriptPreview,
              defaultOpen: true,
              body: transcriptItems.length === 0
          ...
          <div class="rail-column">
            <section class="card stack">
              <div class="row wrap">
                <h2>Requests</h2>
                <div class="timeline-meta">${openRequestCount} open/pending</div>
              </div>
            ...
            <section class="card stack">
              <div class="row wrap">
                <div>
                  ...
                  <div class="timeline-meta">${this.actionViewLabel(this.actionView)}</div>
                </div>
                <div class="timeline-meta">${visibleActionItems.length} visible / ${actionCounts.all} total</div>
              </div>
              <div class="callout small">
                ${this.actionViewDescription(this.actionView)}
              </div>
              <div class="button-row">
                <button class=${this.actionView === "sync" ? "button" : "button secondary"} type="button" @click=${() => { this.actionView = "sync"; }}>
                  Sync Logs (${actionCounts.sync})
                </button>
                <button class=${this.actionView === "followups" ? "button" : "button secondary"} type="button" @click=${() => { this.actionView = "followups"; }}>
                  Follow-ups (${actionCounts.followups})
                </button>
                <button class=${this.actionView === "notes" ? "button" : "button secondary"} type="button" @click=${() => { this.actionView = "notes"; }}>
                  Notes (${actionCounts.notes})
                </button>
                <button class=${this.actionView === "all" ? "button" : "button secondary"} type="button" @click=${() => { this.actionView = "all"; }}>
                  All (${actionCounts.all})
                </button>
              </div>
              ${this.actionView === "notes" || this.actionView === "all"
                ? html`
              ...
              ${visibleActionItems.length === 0
                ? html`<div class="empty">No ${this.actionViewLabel(this.actionView).toLowerCase()} right now.</div>`
                : html`<ul class="action-list">${visibleActionItems.map((item) => this.renderActionItem(item))}</ul>`}
            </section>
          </div>
        </div>
      </div>
              ...
              ${observatory.cycleRunAudits.length === 0
                ...
                : observatory.cycleRunAudits.map((item) => html`
                    <div class="artifact-item">
                      <div class="row wrap">
                        ...
                        <div class="pill-summary">
                          ...
                          ${item.guidanceEffectivenessVerdict
                            ...
                            : nothing}
                          ${item.guidanceAdaptationStatus
                            ? html`<span class="chip">adapt:${item.guidanceAdaptationStatus}</span>`
                            : nothing}
                        </div>
                      </div>
                      ...
                      <div class="artifact-meta">
                        adaptationReport=${item.guidanceAdaptationReportPath ?? "none"} · durable=${item.hasDurableGuidanceAdaptationReport ? "yes" : "no"}
                      </div>
                      <div class="artifact-meta">${item.guidanceOutcomeSummary}</div>
                      <div class="artifact-meta">${item.guidanceEffectivenessSummary}</div>
                      <div class="artifact-meta">${item.guidanceAdaptationNote}</div>
